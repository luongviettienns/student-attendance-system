...
-- Insert each role individually to avoid duplicate key errors
...
IF NOT EXISTS (SELECT 1 FROM roles WHERE role_id = 'ROLE_ADVISOR')
BEGIN
    INSERT INTO dbo.roles (role_id, role_name, description, is_active) VALUES
    ('ROLE_ADVISOR', N'Advisor', N'Cố vấn học tập', 1);
END

DECLARE @RoleCount INT = (SELECT COUNT(*) FROM roles WHERE role_id IN ('ROLE_ADMIN', 'ROLE_LECTURER', 'ROLE_STUDENT', 'ROLE_ADVISOR'));
IF @RoleCount = 4
    PRINT '   ✅ 4 roles ready';
ELSE
    PRINT CONCAT('   ⚠️  Only ', CAST(@RoleCount AS VARCHAR), ' roles exist (expected 4)');
GO
...
BEGIN
    IF EXISTS (SELECT 1 FROM roles WHERE role_id = 'ROLE_ADMIN')
    BEGIN
...
BEGIN
    IF EXISTS (SELECT 1 FROM roles WHERE role_id = 'ROLE_LECTURER')
    BEGIN
        INSERT INTO dbo.users (user_id, username, password_hash, email, phone, full_name, role_id, is_active) VALUES
...
BEGIN
    IF EXISTS (SELECT 1 FROM roles WHERE role_id = 'ROLE_STUDENT')
    BEGIN
        INSERT INTO dbo.users (user_id, username, password_hash, email, phone, full_name, role_id, is_active) VALUES
...
BEGIN
    IF EXISTS (SELECT 1 FROM roles WHERE role_id = 'ROLE_ADVISOR')
    BEGIN
        INSERT INTO dbo.users (user_id, username, password_hash, email, phone, full_name, role_id, is_active) VALUES
    ...
    END
    ELSE
        PRINT '   ⚠️  ROLE_ADVISOR not found, skipping USER008';
END
...
-- Check if students and classes exist before creating enrollments
IF EXISTS (SELECT 1 FROM students WHERE student_id IN ('STU001', 'STU002', 'STU003', 'STU004', 'STU005'))
    AND EXISTS (SELECT 1 FROM classes WHERE class_id IN ('CLS001', 'CLS002', 'CLS003', 'CLS004'))
BEGIN
    -- Insert each enrollment individually
    IF NOT EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR001')
    BEGIN
        IF EXISTS (SELECT 1 FROM students WHERE student_id = 'STU005') AND EXISTS (SELECT 1 FROM classes WHERE class_id = 'CLS001')
        BEGIN
            INSERT INTO dbo.enrollments (enrollment_id, student_id, class_id, status, enrollment_status, enrollment_date) VALUES
            ('ENR001', 'STU005', 'CLS001', N'Đang học', 'APPROVED', GETDATE());
        END
    END

    IF NOT EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR002')
    BEGIN
        IF EXISTS (SELECT 1 FROM students WHERE student_id = 'STU005') AND EXISTS (SELECT 1 FROM classes WHERE class_id = 'CLS002')
        BEGIN
            INSERT INTO dbo.enrollments (enrollment_id, student_id, class_id, status, enrollment_status, enrollment_date) VALUES
            ('ENR002', 'STU005', 'CLS002', N'Đang học', 'APPROVED', GETDATE());
        END
    END

    IF NOT EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR003')
    BEGIN
        IF EXISTS (SELECT 1 FROM students WHERE student_id = 'STU004') AND EXISTS (SELECT 1 FROM classes WHERE class_id = 'CLS003')
        BEGIN
            INSERT INTO dbo.enrollments (enrollment_id, student_id, class_id, status, enrollment_status, enrollment_date) VALUES
            ('ENR003', 'STU004', 'CLS003', N'Đang học', 'APPROVED', GETDATE());
        END
    END

    IF NOT EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR004')
    BEGIN
        IF EXISTS (SELECT 1 FROM students WHERE student_id = 'STU003') AND EXISTS (SELECT 1 FROM classes WHERE class_id = 'CLS004')
        BEGIN
            INSERT INTO dbo.enrollments (enrollment_id, student_id, class_id, status, enrollment_status, enrollment_date) VALUES
            ('ENR004', 'STU003', 'CLS004', N'Đang học', 'APPROVED', GETDATE());
        END
    END

    IF NOT EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR005')
    BEGIN
        IF EXISTS (SELECT 1 FROM students WHERE student_id = 'STU001') AND EXISTS (SELECT 1 FROM classes WHERE class_id = 'CLS004')
        BEGIN
            INSERT INTO dbo.enrollments (enrollment_id, student_id, class_id, status, enrollment_status, enrollment_date) VALUES
            ('ENR005', 'STU001', 'CLS004', N'Đang học', 'APPROVED', GETDATE());
        END
    END

    IF NOT EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR006')
    BEGIN
        IF EXISTS (SELECT 1 FROM students WHERE student_id = 'STU002') AND EXISTS (SELECT 1 FROM classes WHERE class_id = 'CLS003')
        BEGIN
            INSERT INTO dbo.enrollments (enrollment_id, student_id, class_id, status, enrollment_status, enrollment_date) VALUES
            ('ENR006', 'STU002', 'CLS003', N'Đang học', 'APPROVED', GETDATE());
        END
    END

    DECLARE @EnrollmentCount INT = (SELECT COUNT(*) FROM enrollments WHERE enrollment_id IN ('ENR001', 'ENR002', 'ENR003', 'ENR004', 'ENR005', 'ENR006'));
    IF @EnrollmentCount = 6
        PRINT '   ✅ 6 enrollments created';
    ELSE
        PRINT CONCAT('   ⚠️  Only ', CAST(@EnrollmentCount AS VARCHAR), ' enrollments created (expected 6)');
END
ELSE
BEGIN
    PRINT '   ⚠️  Cannot create enrollments: students or classes not found';
END
GO
...
-- Check if enrollments exist before creating grades
IF EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id IN ('ENR001', 'ENR002', 'ENR003', 'ENR004', 'ENR005', 'ENR006'))
BEGIN
    -- Insert each grade individually
    IF NOT EXISTS (SELECT 1 FROM grades WHERE grade_id = 'GRD001')
    BEGIN
        IF EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR001')
        BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM grades WHERE grade_id = 'GRD002')
    BEGIN
        IF EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR002')
        BEGIN
            INSERT INTO dbo.grades (grade_id, enrollment_id, midterm_score, final_score, total_score, letter_grade) VALUES
    ...
    IF NOT EXISTS (SELECT 1 FROM grades WHERE grade_id = 'GRD003')
    BEGIN
        IF EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR003')
        BEGIN
            INSERT INTO dbo.grades (grade_id, enrollment_id, midterm_score, final_score, total_score, letter_grade) VALUES
    ...
    IF NOT EXISTS (SELECT 1 FROM grades WHERE grade_id = 'GRD004')
    BEGIN
        IF EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR004')
        BEGIN
            INSERT INTO dbo.grades (grade_id, enrollment_id, midterm_score, final_score, total_score, letter_grade) VALUES
    ...
    IF NOT EXISTS (SELECT 1 FROM grades WHERE grade_id = 'GRD005')
    BEGIN
        IF EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR005')
        BEGIN
            INSERT INTO dbo.grades (grade_id, enrollment_id, midterm_score, final_score, total_score, letter_grade) VALUES
    ...
    IF NOT EXISTS (SELECT 1 FROM grades WHERE grade_id = 'GRD006')
    BEGIN
        IF EXISTS (SELECT 1 FROM enrollments WHERE enrollment_id = 'ENR006')
        BEGIN
            INSERT INTO dbo.grades (grade_id, enrollment_id, midterm_score, final_score, total_score, letter_grade) VALUES
    ...
    DECLARE @GradeCount INT = (SELECT COUNT(*) FROM grades WHERE grade_id IN ('GRD001', 'GRD002', 'GRD003', 'GRD004', 'GRD005', 'GRD006'));
    IF @GradeCount = 6
        PRINT '   ✅ 6 grades created';
    ELSE
        PRINT CONCAT('   ⚠️  Only ', CAST(@GradeCount AS VARCHAR), ' grades created (expected 6)');
END
ELSE
BEGIN
    PRINT '   ⚠️  Cannot create grades: enrollments not found';
END
GO
...
-- Check if role exists before assigning permissions
IF EXISTS (SELECT 1 FROM roles WHERE role_id = 'ROLE_ADVISOR')
BEGIN
