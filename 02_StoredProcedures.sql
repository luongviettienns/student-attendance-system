...
BEGIN
    ...
    SELECT s.student_id, s.user_id, s.student_code, s.full_name, s.gender, s.date_of_birth,
           s.date_of_birth AS dob, -- alias for DAL mapping
           s.email, s.phone, s.address, s.major_id, m.major_name, m.faculty_id, f.faculty_name,
           s.academic_year_id, ay.year_name, ay.year_name AS year_code, -- alias for DAL mapping
           s.is_active,
...
BEGIN
    SELECT 
        ...
        s.date_of_birth AS dob,                  -- alias for DAL mapping
        m.major_name, 
        m.faculty_id,                            -- expose faculty_id via major
        f.faculty_name, 
        ay.year_name, 
        ay.year_name AS year_code                -- alias for DAL mapping
    FROM dbo.students s
    LEFT JOIN dbo.majors m ON s.major_id = m.major_id
    LEFT JOIN dbo.faculties f ON m.faculty_id = f.faculty_id
    LEFT JOIN dbo.academic_years ay ON s.academic_year_id = ay.academic_year_id
...
-- ===========================================
-- 6A. STUDENTS MANAGEMENT - FULL VARIANT (for DAL compatibility)
-- ===========================================
...
CREATE PROCEDURE sp_AddStudentFull
    ...
    @Email VARCHAR(150) = NULL,
    @Phone VARCHAR(20) = NULL,
    @FacultyId VARCHAR(50) = NULL,         -- not stored (no column); kept for compatibility
    @MajorId VARCHAR(50) = NULL,
    @AcademicYearId VARCHAR(50) = NULL,
    @CohortYear VARCHAR(10) = NULL,        -- not stored (no column); kept for compatibility
    @CreatedBy VARCHAR(50) = 'system'
AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @StudentId VARCHAR(50) = NEWID();
    ...
    VALUES (
        @StudentId, @UserId, @StudentCode, @FullName, @Gender, @Dob,
        @Email, @Phone, NULL, @MajorId, @AcademicYearId,
        GETDATE(), @CreatedBy
    );
...
CREATE PROCEDURE sp_UpdateStudentFull
    ...
    @AcademicYearId VARCHAR(50) = NULL,
    @CohortYear VARCHAR(10) = NULL,        -- not stored (no column); kept for compatibility
    @Nationality NVARCHAR(50) = NULL,      -- profile fields not stored; ignored
    ...
    @FamilyFullName NVARCHAR(150) = NULL,  -- family fields not stored; ignored
    @RelationType NVARCHAR(30) = NULL,
    @BirthYear INT = NULL,
    @PhoneFamily NVARCHAR(20) = NULL,
    @JobFamily NVARCHAR(150) = NULL,
    @UpdatedBy VARCHAR(50) = 'system'
AS
BEGIN
    SET NOCOUNT ON;
...
-- Delete Student Full (soft delete + related cleanup similar to sp_DeleteStudent)
...
BEGIN
    ...
    -- Remove related attendances and grades first
    DELETE a
    FROM attendances a
