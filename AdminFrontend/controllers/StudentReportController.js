...
app.controller('StudentReportController', ['$scope', 'ReportService', 'SchoolYearService', 'ToastService', 'LoggerService', 'AuthService',
    ...
    $scope.loadReports = function() {
        ...
        ReportService.getStudentReports($scope.filters).then(function(res) {
            ...
            // Map data to scope
            $scope.reports.overview = data.overview || {};
            $scope.reports.gpaTrend = data.gpaTrend || [];
            $scope.reports.gradeDistribution = data.gradeDistribution || { A: 0, B: 0, C: 0, D: 0, F: 0 };
            $scope.reports.creditDebt = data.creditDebt || { total: 0, requiredCredits: 120, progress: 0 };
    ...
    $scope.getCreditProgress = function() {
        ...
        return Math.round(($scope.reports.creditDebt.creditsEarned / $scope.reports.creditDebt.requiredCredits) * 100);
    };
